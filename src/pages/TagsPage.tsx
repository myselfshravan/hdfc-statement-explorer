...
export function TagsPage() {
  ...
  const [editingTag, setEditingTag] = useState<TagWithTransactions | null>(
    null
  );
  ...
  const [deletingTag, setDeletingTag] = useState<TagWithTransactions | null>(
    null
  ); // Still needed for context in delete dialog
  ...
  const handleCreateTag = async () => {
    // Removed closeDialog parameter
    if (!newTagName.trim()) return;
    setIsCreating(true);
    setCreateError(null);
    try {
      const newTagData = await tagManager.createTag(
        newTagName.trim(),
        selectedColor
      );
      setTags((prevTags) => [
        ...prevTags,
        { ...newTagData, transaction_count: 0 },
      ]);
  ...
  const handleDeleteTag = async (onSuccessClose?: () => void) => {
    ...
    try {
      ...
      setTags((prevTags) =>
        prevTags.filter((tag) => tag.id !== deletingTag.id)
      );
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Tags are shared across all users. Please ensure the tag name
                  is clear and meaningful for everyone.
                </p>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <Input
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => prepareToDeleteTag(tag)}
                          >
                            Delete
                          </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              <span className="font-semibold">
                                {deletingTag?.id === tag.id ? tag.name : ""}
                              </span>
                              ".{" "}
                              {/* Show name only if it's the one being deleted */}
                              {deletingTag?.id === tag.id &&
                                deletingTag.transaction_count > 0 && (
                                  <span className="block text-yellow-600 mt-2">
                                    This tag is assigned to{" "}
                                    {deletingTag.transaction_count}{" "}
                                    transaction(s).
                                  </span>
                                )}
                              {deleteError && deletingTag?.id === tag.id && (
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              ...
                              } // Disable only the specific action button
